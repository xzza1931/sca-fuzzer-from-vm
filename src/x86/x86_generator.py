    ...
    def sandbox_memory_access(self, instr: Instruction, parent: BasicBlock):
        ...
        if mem_operands:
            ...
            for address_reg, mem_operand in uniq_operands.items():
                if instr.name == "call" or instr.name == "ret":
                    print("call or ret instruction found; skipping sandboxing")
                else:
...
class X86RandomGenerator(X86Generator, RandomGenerator):
    ...
    def mutate_for_discarded_test_case(self, discarded: str, enable_spec_mutate: bool = True) -> TestCase:
        """
        对被淘汰的用例做复活变异
        ...
        :return: 变异后的测试用例
        """
        #breakpoint()          # 内置调试器，等价于 pdb.set_trace()
        print(f"=============================Mutating discarded test case: {discarded}")
        ...
        print(f"enable_spec_mutate:{enable_spec_mutate}")
        if enable_spec_mutate:
            print("Speculation mutation enabled.")
    ...
    def mutate_speculation_s1(self, tc: TestCase) -> None:
        ...
        :return: None
        """
        print("Applying speculation mutation S1...")
