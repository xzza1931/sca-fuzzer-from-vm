...
class InstructionSpec:
    ...
    def dump_to_file(self, f):
        """
        将当前 InstructionSpec 对象的主要属性写入已打开的文件句柄 f。
        f: 需要是一个以文本写入模式打开的文件对象
        """
        f.write("=====================================\n")
        f.write(f"Instruction name: {getattr(self, 'name', None)}\n")
        f.write(f"Category: {getattr(self, 'category', None)}\n")
        f.write(f"Control flow: {getattr(self, 'control_flow', None)}\n")
        f.write(f"Has memory operand: {getattr(self, 'has_mem_operand', None)}\n")
        f.write(f"Has write: {getattr(self, 'has_write', None)}\n")
        f.write(f"Has magic value: {getattr(self, 'has_magic_value', None)}\n")
        ...
        # 打印隐式操作数
        f.write("Implicit operands:\n")
        for op in getattr(self, "implicit_operands", []):
            f.write(f"    {str(op)}\n")
...
class Instruction:
    ...
    def dump_to_file(self, f):
        """
        将 Instruction 对象的详细信息写入到一个已经打开的文件中。
        参数:
        f: 已经以写模式打开的文件对象
        """
        f.write("=====================================\n")
        f.write(f"Instruction name: {self.name}\n")
        f.write(f"Category: {self.category}\n")
        f.write(f"Control flow: {self.control_flow}\n")
        f.write(f"Is instrumentation: {self.is_instrumentation}\n")
        f.write(f"Is from template: {self.is_from_template}\n")
        f.write(f"Is no-remove: {self.is_noremove}\n")
        f.write(f"Section ID: {self.section_id}\n")
        f.write(f"Section offset: {self.section_offset}\n")
        f.write(f"Line number: {self.line_num}\n")
        f.write(f"Size (bytes): {self.size}\n")

        # 简要表示（便于查看）
        f.write(f"Brief representation: {self.get_brief()}\n")

        # 打印显式操作数
        f.write("Operands:\n")
        if self.operands:
            for o in self.operands:
                f.write(f"    {self.format_operand(o)}\n")
        else:
            f.write("    (none)\n")
        ...
        # 打印寄存器 / 内存 / 立即数信息
        f.write("Register operands:\n")
        regs = self.get_reg_operands()
        for r in regs:
            f.write(f"    {self.format_operand(r)}\n")

        f.write("Memory operands:\n")
        mems = self.get_mem_operands()
        for m in mems:
            f.write(f"    {self.format_operand(m)}\n")

        f.write("Immediate operands:\n")
        imms = self.get_imm_operands()
        for i in imms:
            f.write(f"    {self.format_operand(i)}\n")

        f.write("=====================================\n\n")

    def format_operand(self, o):
        try:
            # 优先返回 value
            if hasattr(o, "value"):
                return str(o.value)
            ...
            # 对标志寄存器单独处理
            elif hasattr(o, "flags"):
                return f"{'|'.join(o.flags)}"

            # 对 LabelOperand
            elif hasattr(o, "label"):
                return str(o.label)

            # 对寄存器操作数
            elif hasattr(o, "reg_name"):
                return str(o.reg_name)

            # 对 MemoryOperand
            elif hasattr(o, "base") or hasattr(o, "addr"):
                parts = []
                if getattr(o, "base", None):
                    parts.append(o.base)
                if getattr(o, "index", None):
                    parts.append(f"{o.index}*{getattr(o, 'scale', 1)}")
                if getattr(o, "disp", 0):
                    parts.append(f"{hex(o.disp)}")
                return "[" + "+".join(parts) + "]"

            else:
                return str(o)
        except Exception as e:
            return f"<Error formatting operand: {e}>"
